...
import type { BenchPublishOptions, CommandType, FormatType, PublishOptions, SimulatePubOptions, Simulator } from 'mqttx'
...
import { basicLog, benchLog, Signale, signale, simulateLog } from '../utils/signale'
import { loadSimulator } from '../utils/simulate'

function processPublishMessage(message: string | Buffer, protobufPath: string | undefined, protobufMessageName: string | undefined, format: FormatType | undefined): Buffer | string {
  /*
...
function send(config: boolean | string | undefined, connOpts: IClientOptions, pubOpts: {
  topic: string
...
function multisend(config: boolean | string | undefined, connOpts: IClientOptions, pubOpts: {
  topic: string
  message: string | Buffer
  protobufPath: string | undefined
  protobufMessageName: string | undefined
  ...
  sender._write = (line, _enc, cb) => {
    ...
    count++
    const omitTopic = opts.properties?.topicAlias && count >= 2
...
function pub(options: PublishOptions) {
...
async function multiPub(commandType: CommandType, options: BenchPublishOptions | SimulatePubOptions) {
  const { save, config } = options
...
async function simulatePub(options: SimulatePubOptions) {
  multiPub('simulate', options)
}

export default pub

export { benchPub, pub, simulatePub }
